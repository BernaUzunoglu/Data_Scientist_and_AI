...
# Sampling (Örnekleme)
############################
#  Neden popülasyon : Çünkü belirli bir kitleden düzgün bir dağılım ile belirli kişiler içerisinden topluluk hakkında genelleme yapabilme imkanı sağlar.
populasyon = np.random.randint(0, 80, 10000)  # 0 -80 arasında 10000 tane rasgele sayı
populasyon.mean()

# kodu çalıştırdırdığımızda aynı rasgele sayılara ulaşmak için.
np.random.seed(115)
...
#   Birden fazla örneklem çekip bunların da ortalamasını aldığınızda daha kuvvetli bir çıkarım gerçekleştirebiliyoruz
...
df["total_bill"].mean()
# İlgili değişken verilerin ortalaması için %95 güven aralığını hesaplar.
...
#  Verilen bahşiş ile ödenen hesap arasındaki korelasynun incelenmesi.
df["total_bill"] = df["total_bill"] - df["tip"]

#  Saçılım grafiği oluşturalım.
df.plot.scatter("tip", "total_bill")
plt.show()

# Correlation değerini bulma
...
# matematiksel olarak sigara içenlerin ve içmeyenlerin arasında fark var gibi duruyor.
...
# Normallik Varsayımı : Bir değişkenin dağılımının standart normal dağılıma benzer olup olmadığının aslında hipotez testidir
# Varyans Homojenliği
...
# shapiro bir değişkenin dağılımının normal olup olmadığını test eder.
test_stat, pvalue = shapiro(df.loc[df["smoker"] == "Yes", "total_bill"])
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
#  Birinci grup için p-value değeri 0.0002 çıktığı için H0 reddedilir. Yani normal dağımım yoktur.
...
# değişkenlerin homojenliği incelenmek için levene kullanılır. varyans homojenliği olup olmadığı
test_stat, pvalue = levene(df.loc[df["smoker"] == "Yes", "total_bill"],
                           df.loc[df["smoker"] == "No", "total_bill"])
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# p-value = 0.0452 değeri 0.05 den küçük olduğu için H0 reddediyoruz. Varyanslar homojen değildir yorumunu yapıyoruz.
...
############################
#  ttest_ind
# normalliik varsayımı varsa kullan
#  ttest_ ind normallik,varyans homojenlik varsayımı varsa beni kullanabilirsin der.
#  normallik varsayımı var varyans homojen varsayımı yoksa yine kullan  equal_var = False yap
...
# Sonuç : iki grup içinde normallik varsayımı sağlanmamaktadır. H0 reddedlir.
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue)) #  Varyanslar homojendir.
...
df = pd.read_csv("Measurement_Problems/datasets/diabetes.csv")
df.head()
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# Kurulan hipotezi reddederiz. Kursu izleyenler ile izlemeyenler arasında ist. bir fark vardır yorumunu yapıyoruz.
